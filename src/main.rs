...
fn add (args: &mut std::iter::Skip<std::env::Args>) {
    ...
    let list_name = if list_name_input.starts_with("#") {list_name_input[1..].to_string()} else {list_name_input};

    let item_path_components = item_path.split("/");

    let parents = item_path_components.clone().take(item_path_components.clone().count() - 1);
