...
pub fn save_list(data: list::List) {
    std::fs::write(data.path.clone().as_path(), list_to_string(data))
        .expect("should have been able to save the '.todo' file at {path}");
}
    ...
    for item in items {
        ...
        if item.items.len() > 0 {
            output += &format!(
                ...
                children = items_to_string(item.items, depth + 1)
            );
        } else {
